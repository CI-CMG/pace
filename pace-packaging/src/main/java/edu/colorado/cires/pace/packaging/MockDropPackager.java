 ...
 */
class MockDropPackager {

  /**
   * Packages data an intentionally uses incorrect file path
   ...
   */
  public static void run(int successfulAttemptNumber, Stream<PackageInstruction> moveInstructions, Path outputDir, List<Package> packages, List<Person> people, Logger logger, ProgressIndicator... progressIndicators) throws PackagingException {
    mkdir(outputDir);
    ...
    Path manifestFile = copyFilesAndWriteManifest(
        moveInstructions, outputDir, incrementProgressFn, logger, successfulAttemptNumber
    );
  ...
  protected static Path copyFilesAndWriteManifest(Stream<PackageInstruction> moveInstructions, Path outputDir, Runnable incrementProgressFn,
      Logger logger, int successfulAttemptTimer) throws PackagingException {
      ...
      moveInstructions
          ...
          .forEach(packageInstruction -> {
            ...
            while(true){
              IOException output;
              try {
                output = copyFileAttempt(packageInstruction, logger, i, successfulAttemptTimer);
            ...
            while(true){
              ...
              try {
                output = appendToManifestAttempt(packageInstruction, logger, writer,
                    outputDir, outputFile, incrementProgressFn, i, successfulAttemptTimer);
              } catch (InterruptedException e) {
                throw new RuntimeException(e);
              }
              if (output == null){
  ...
  private static IOException appendToManifestAttempt(PackageInstruction packageInstruction, Logger logger, FileWriter writer, Path outputDir,
      Path outputFile, Runnable incrementProgressFn, int i, int successfulAttemptTimer) throws InterruptedException {
    try {
      if (i < successfulAttemptTimer) {
        Path fakeOutputFile = Paths.get(packageInstruction.target().toString().replace("target","fakeDir"));
        FileUtils.appendChecksumToManifest(writer, fakeOutputFile, outputDir);
      } else {
  ...
  private static IOException copyFileAttempt(PackageInstruction packageInstruction, Logger logger, int i, int successfulAttemptNumber) throws InterruptedException {
    try {
      Path source = packageInstruction.source();
      if (i >= successfulAttemptNumber){
        source = Paths.get(packageInstruction.source().toString().replace("***", ""));
      }
      FileUtils.copyFile(source, packageInstruction.target());
