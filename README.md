...
The Passive Acoustic Collection Engine (PACE) tool provides a programmatic approach to packaging passive acoustic data for submission to NCEI. PACE is designed for users with a  need to submit large numbers of datasets to NCEI and/or users who manage metadata in a system such as Tethys or a relational database. PACE can be controlled via a graphical user interface (GUI) or from the command line (CLI) and supports user-created metadata spreadsheets or direct integration with database systems. **We anticipate the GUI version will be the most widely used and recommend starting there.** PACE creates data packages like those created by [PassivePacker](https://www.ncei.noaa.gov/products/passivepacker) without the need for PassivePacker’s manual metadata entry.
...
## Releases
- Latest Release: [1.2.4](https://github.com/CI-CMG/pace/releases/tag/v1.2.4)
...
2. Look under the most recent release version which is not trunk-latest (currently v1.2.4)
...
### Installation
#### MacOS
1. Download file using ``curl``: ``curl -L -O https://github.com/CI-CMG/pace/releases/download/trunk-latest/pace-cli-macOS-ARM64-1.2.4-SNAPSHOT.pkg``
2. Use ``installer`` to install ``pace-cli`` (requires sudo): ``installer -pkg pace-cli-macOS-ARM64-1.2.4-SNAPSHOT.pkg -target /``
...
#### Linux
1. Download file using ``curl``: ``curl -L -O https://github.com/CI-CMG/pace/releases/download/trunk-latest/pace-cli-Linux-X64-1.2.4-SNAPSHOT.zip``
2. Unzip file: ``unzip pace-cli-Linux-X64-1.2.4-SNAPSHOT.zip``
...
#### Windows
1. Download file using `curl`: `curl -L -O https://github.com/CI-CMG/pace/releases/download/trunk-latest/pace-cli-Windows-X64-1.2.4-SNAPSHOT.msi`
2. Execute downloaded file and follow installer prompts: `msiexec \i  pace-cli-Windows-X64-1.2.4-SNAPSHOT.msi`
...
- Updated audio file filtering, upgraded dependencies with vulnerabilites, and added validation for quality entries
